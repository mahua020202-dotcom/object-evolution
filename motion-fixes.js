(()=>{
  ...
  function sync(){
    ...
    if(ai){const ar=ai.getBoundingClientRect();const ap=clamp((innerHeight-ar.top)/(innerHeight+Math.max(ai.offsetHeight*.42,1)));ai.style.setProperty('--ai-progress',ap.toFixed(3));ai.classList.toggle('ai-enter',ap>.08);ai.classList.toggle('ai-focus',ap>.42);ai.classList.toggle('ai-intent',ap>.72)}
  }
  addEventListener('scroll',sync,{passive:true});addEventListener('resize',sync);addEventListener('load',sync);sync();
})();
