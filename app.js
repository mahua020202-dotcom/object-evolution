...
// Preload every exhibit so a generation change never flashes an empty frame.
...
function setEra(index,local,global){
  ...
  if(index===ERAS.length-1){
    if(productImage.dataset.src!==to.image){productImage.dataset.src=to.image;productImage.src=to.image}
